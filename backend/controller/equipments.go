...
func GetAllEquipments(c *gin.Context) {
    ...
    //แต่ละรายการจะถูกฟอร์แมตให้เรียบร้อย แล้วส่งกลับไปยังผู้ใช้ 
...
func GetEquipment(c *gin.Context) {
    ID := c.Param("id")    //ดึงค่าพารามิเตอร์ id จาก URL 
...
func UpdateEquipment(c *gin.Context) {
    ...
    result := db.First(&equipment, EquipmentID) //ดึงตาม id
    if result.Error != nil {
        c.JSON(http.StatusNotFound, gin.H{"error": "id not found"})
        return
    }

    //จับคู่โครงสร้าง Json ที่ส่งมาเข้ากับโครงสร้างข้อมูล
